'''
Copy from netmiko base_connection to customize use
...
from netconnect.extends.utilities import write_bytes, string_types
from netconnect.extends.globals import MAX_BUFFER, BACKSPACE_CHAR
...
class BaseConnection(object):
    ...
    def _timeout_exceeded(self, start, msg='Timeout exceeded!'):
        """Raise SSHException if waiting too much in the serving queue.
        ...
        while i < max_loops:
            if self.protocol == 'ssh':
                try:
                    ...
                    raise SSHException("Timed-out reading channel, data not available.")
                finally:
                    self._unlock_netmiko_session()
            ...
            i += 1
        raise SSHException("Timed-out reading channel, pattern not found in output: {}"
                                      .format(pattern))
    ...
    def establish_connection(self, width=None, height=None):
        ...
        Timeout will generate a SSHException
        Authentication failure will generate a AuthenticationException
    ...
    def _test_channel_read(self, count=40, pattern=""):
        ...
        if new_data:
            ...
            raise SSHException("Timed out waiting for data")
    ...
    def send_config_set(self, config_commands=None, exit_config_mode=True, delay_factor=1,
                        ...
                        config_mode_command=None, pattern=''):
        """
        ...
        if not pattern:
            ...
            pattern = re.escape(self.find_prompt())
            output += self._read_channel_expect(pattern=pattern)
        if exit_config_mode:
